...
export const saveMessages = (messages) => {
  try {
    const storageKey = getPlaygroundStorageKey(STORAGE_KEYS.MESSAGES);
    if (!storageKey) {
      return;
    }
...
export const clearConfig = () => {
  try {
    const configKey = getPlaygroundStorageKey(STORAGE_KEYS.CONFIG);
    const messagesKey = getPlaygroundStorageKey(STORAGE_KEYS.MESSAGES);
    if (configKey) {
      localStorage.removeItem(configKey);
    }
    if (messagesKey) {
      localStorage.removeItem(messagesKey); // 同时清除消息
    }
  } catch (error) {
